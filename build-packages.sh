...
# OpenWrt's package layout: root/ maps to /, LuCI htdocs maps to /www/.
cp -a root/. "$DATA_DIR/"
if [ -d htdocs ]; then
...
# The package contains no compiled target code, so preserve shell executable bits.
...
EOF
    ...
    (cd "$ipk_dir" && "$TAR" --format=gnu --numeric-owner --owner=0 --group=0 -cf - \
        ./debian-binary ./data.tar.gz ./control.tar.gz | gzip -n > "$OLDPWD/$DIST_DIR/$IPK_FILE")
}

make_apk() {
    command -v docker >/dev/null 2>&1 || {
        echo "Docker is required to build APK" >&2
        exit 1
    }
